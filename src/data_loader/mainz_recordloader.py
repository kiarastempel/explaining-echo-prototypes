...
def build_dataset_validation(file_names, target):
    return build_dataset(file_names, 1, None, None, target, False)


def build_dataset(file_names, batch_size, shuffle_size, number_of_input_frames, target, augment=False):
    ...
    if augment:
        ds = ds.map(lambda x: parse_and_augment_example(x, number_of_input_frames,target),
                    num_parallel_calls=AUTOTUNE)
    else:
        ds = ds.map(lambda x: parse_example(x, number_of_input_frames, target),
                    num_parallel_calls=AUTOTUNE)
    ds = ds.batch(batch_size=batch_size, drop_remainder=True)
    return ds.prefetch(AUTOTUNE)


def parse_example(example, number_of_input_frames, target):
    ...
    return subframes, parsed_example[target]


def parse_and_augment_example(example, number_of_input_frames, target):
    parsed_example = tf.io.parse_example(example, feature_description)
    number_of_frames = parsed_example['number_of_frames']
    raw_frames = tf.sparse.to_dense(parsed_example['frames'])
