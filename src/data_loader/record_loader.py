...
def build_dataset(file_names, batch_size, shuffle_size, number_of_input_frames, width, height, augment=False,
                  split=False):
    ...
    if split:
        ds = ds.map(lambda x: parse_and_augment_example(x, number_of_input_frames, width, height),
                    num_parallel_calls=AUTOTUNE)
    else:
        ds = ds.map(lambda x: parse_example(x, number_of_input_frames, width, height),
                    num_parallel_calls=AUTOTUNE)
...
def parse_example(example, number_of_input_frames, width, height):
...
def parse_and_augment_example(example, number_of_input_frames, width, height):
    parsed_example = tf.io.parse_example(example, feature_description)
    channels = 1
    number_of_frames = parsed_example['number_of_frames']
    raw_frames = tf.sparse.to_dense(parsed_example['frames'])
